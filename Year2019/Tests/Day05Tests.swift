...
final class Day05Tests: XCTestCase {
    ...
    func testPart2Example1() throws {
        ...
        XCTAssertEqual(try IntcodeComputer(code: code, input: 8).run().value, 1)
        XCTAssertEqual(try IntcodeComputer(code: code, input: 7).run().value, 0)
    }

    func testPart2Example2() throws {
        let code = [3,9,7,9,10,9,4,9,99,-1,8]
        XCTAssertEqual(try IntcodeComputer(code: code, input: 7).run().value, 1)
        XCTAssertEqual(try IntcodeComputer(code: code, input: 8).run().value, 0)
    }

    func testPart2Example3() throws {
        let code = [3,3,1108,-1,8,3,4,3,99]
        XCTAssertEqual(try IntcodeComputer(code: code, input: 8).run().value, 1)
        XCTAssertEqual(try IntcodeComputer(code: code, input: 9).run().value, 0)
    }
    ...
    func testPart2Example5() throws {
        ...
        XCTAssertEqual(try IntcodeComputer(code: code, input: 0).run().value, 0)
        XCTAssertEqual(try IntcodeComputer(code: code, input: 8).run().value, 1)
    }
    ...
    func testPart2Example7() throws {
        ...
        XCTAssertEqual(try IntcodeComputer(code: code, input: 7).run().value, 999)
        XCTAssertEqual(try IntcodeComputer(code: code, input: 8).run().value, 1000)
        XCTAssertEqual(try IntcodeComputer(code: code, input: 9).run().value, 1001)
    }
