    ...
    public func part2(input: Input) throws -> Int {
        ...
        while let position = positions.first {
            ...
            case (.pulled, false):
                ...
                positions.append(position + Vector(dx: 1, dy: 0))
                guard try drone(at: position + Vector(dx: 0, dy: 99)) == .pulled else { continue }
                positions.insert(position + Vector(dx: 0, dy: 1), at: 0)

            // We're inspecting the beam.
            case (.pulled, true):
                positions.append(position + Vector(dx: 0, dy: 1))
            ...
            guard try drone(at: position + Vector(dx: 99, dy:  0)) == .pulled else { continue }
            guard try drone(at: position + Vector(dx:  0, dy: 99)) == .pulled else { continue }
